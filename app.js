...
let brandname = Object.keys(mobiles);

for (var i = 0; i< brandname.length ; i++){
    // console.log(brandname[i])
brandSelect.innerHTML += `<option>${brandname[i]}</option>`
}

function selectbrand(){
    let brandvalue = brand.value
    let modelname = Object.keys(mobiles[brandvalue]);
    model.innerHTML = ""
    for (var i = 0; i< modelname.length ; i++){
        model.innerHTML += `<option>${modelname[i]}</option>`
        }
        // console.log("allModels==>",modelname)
}
...
function search(){
  ...
  <div class="card-body">
    <h5 class="card-title">${brand.value}</h5>
